...
const TagsPage = () => {
  ...
  const deleteTagHandler = (id, isUsed) => {
    openModal(
      "Czy jestes pewien?",
      isUsed
        ? "Tag który chcesz usunąć jest obecnie używany, czy jesteś pewien że chcesz go usunąć ?"
    ...
    <div className="flex flex-col gap-1 px-10 py-3 ">
      <div className="px-0.5 pt-2 mb-10 flex items-center gap-2 justify-between ">
        <h2 className="text-xl font-bold text-gray-600 flex items-center gap-2 ">
          <HiOutlineHashtag className="text-blue-900" />
          Lista Tagów
        </h2>
        <div className=" mx-5 ">
          <Button
            onClick={() => {
              openContentModal({
                ...
                title: "Dodaj tag",
                description:
                  "Wypełnij wymagane dane i zatwierdź, aby dodać nowy tag",
                content: <AddTag />,
          ...
          return (
            ...
            >
              <div className="flex justify-between">
                <span className="font-semibold text-gray-700/90">
                  {tag?.name}
                </span>
