import { ArticleListItem, ArticlesFilter } from "@/components/articles/views/feedView/ArticlesFeedView";
...
const {data:favouriteArticles=[],isLoading}= useQuery({
    queryKey:['favourites'],
    queryFn:()=>{
        return userApi.findAllFavouriteArticles();
    }
})
...
if(favouriteArticles?.data && favouriteArticles?.data?.length===0){
    return (
      ...
      <p className="text-base  max-w-[360px] text-primary-foreground/75">Wygląda na to że twoja lista ulubionych artykułów jest pusta.</p>
      </div>
            <Link
             to="/articles"
    ...
    return <div className=" text-foreground p-5 h-full flex w-full max-w-[1540px] mx-auto gap-6 ">
     
        <ArticlesFilter />
            
   
                 <div className="text-foreground flex flex-col p-3 h-full flex w-full max-w-[1540px] mx-auto gap-1">
                    <h2 className="flex items-center gap-1.5 mb-6 text-xl font-semibold"><FaStar/>Moje ulubione</h2>
