  ...
  openContentModal(
    {
      ...
      content:<EditArticle type={id ? "view" :"modal"} article={article} />,size:"xl" })
}
...
const articleDropdownOptions= [
  {label:`${article?.isFavourite ? "Usuń z ulubionych":"Dodaj do ulubionych"}`, icon: article?.isFavourite ? <FaStar /> : <FaRegStar />, actionHandler: () => markAsFavouriteHandler({id})},
...
<div className='py-2.5 px-0.5 space-x-1 space-y-1.5'>
      ...
      {article?.tags?.map((tag)=>{
          return(
              <BadgeLabel className="bg-badge_primary rounded-md" label={tag?.name}/>
          )
      })}
...
<span className="font-semibold flex items-center gap-x-1.5 text-sm text-slate-600">
                  ...
                  <span className='text-sm'>Dodano</span>{formatDate(article?.createdAt)} 
                </span>
</div>
    ...
    {article?.isFavourite &&<FaStar className='w-5 h-5 text-amber-400/90'/>}
    ...
    <span className='py-3.5 px-5 text-sm text-slate-600 font-semibold flex items-center gap-1.5'>
      <div className='border rounded-full w-7 h-7 p-2 flex items-center justify-center bg-violet-500 text-white'>AH</div>
