    ...
    return (
        <div className="min-h-screen px-6 pb-6 flex flex-col space-y-6 relative">
            ...
            <div className="flex items-center px-6 space-x-3">
                <Skeleton animation="none" className="w-10 h-10 rounded-full" />
                <Skeleton animation="none" className="w-32 h-6 rounded-md animate-nonne" />
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 flex flex-col space-y-6">
                    {/* Banner Section */}
                    <Skeleton animation="none" className="w-full h-40 rounded-md " />
                    <div className="flex space-x-4 px-6">
                        <Skeleton animation="none" className="w-24 h-24 rounded-lg " />
                        <Skeleton animation="none" className="w-32 h-8 rounded-md " />
                    </div>
                    <Skeleton animation="none" className="w-full h-10 rounded-md px-6 " />
                    {/* Tags Section */}
                    <div className="flex flex-wrap gap-2">
                        {[...Array(4)].map((_, index) => (
                            <Skeleton animation="none" key={index} className="w-20 h-6 rounded-lg " />
                        ))}
                    </div>
                    {/* Accordions */}
                    <Skeleton animation="none" className="w-full h-28 rounded-lg " />
                    <Skeleton animation="none" className="w-full h-56 rounded-lg " />
                </div>

                {/* Right Sidebar */}
                <div className="sticky top-16 space-y-6">
                    <Skeleton animation="none" className="w-full h-96 rounded-lg " />
                    <Skeleton animation="none" className="w-full h-20 rounded-lg " />
                    {[...Array(4)].map((_, index) => (
                        <Skeleton animation="none" key={index} className="w-full h-10 rounded-md " />
                    ))}
                </div>
            </div>
        </div>
