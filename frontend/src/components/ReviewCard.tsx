...
const ReviewCard: React.FC = ({ article, selectedArticle, handleCloseCard }) => {
    ...
    const { mutate: restoreDeletedArticleHandler, isLoading } = useMutation({
        mutationFn: ({ id }) => {
            return articleApi.restoreArticle({ id });
        },
        onSuccess: (data) => {
    ...
    const articleDropdownOptions = [
        {
            ...
            actionHandler: ({ article, isUsed }) => restoreArticleHandler({ id: article?._id, isUsed }),
        },
    ...
    return (
        ...
        >
            ...
            <div className={`p-4 border-b ${isSelected ? "border-blue-200" : "border-gray-100"}`}>
