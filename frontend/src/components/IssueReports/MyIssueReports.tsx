...
const MyIssueReports = () => {
    const [type, setType] = useState<"all" | "pending" | "resolved" | "rejected">("");
    ...
    const {
        ...
        queryKey: ["myIssueReports", type],
        queryFn: () => {
            return issueReportApi.findMyIssueReports({ type });
        },
    });
    ...
    // const filteredReports = myIssueReports?.filter((report) => type === "all" || report.status === type);
    ...
    return (
        <div className="mx-auto space-y-4  bg-background w-full h-full">
            ...
            {myIssueReports?.length === 0 && !isLoading && (
